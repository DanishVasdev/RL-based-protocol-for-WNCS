...
class ZWPIDControllerApp : public ApplicationBase, public UdpSocket::ICallback
{
  protected:
    UdpSocket socket;
    ...
    std::vector<double> currentState;  // Added state vector as class member
  ...
  public:
    ZWPIDControllerApp()
            : pidCart(192.189, 222.27, 137.44),    // Initialize pidCart with kp=2.0, ki=0.0, kd=50.0
              pidPole(605.48, 219.14, 10.0)    // Initialize pidPole with kp=8.0, ki=0.0, kd=100.0
    {
        currentState.resize(4, 0.0);  // Initialize state vector with zeros
    }
    virtual ~ZWPIDControllerApp();
...
Define_Module(ZWPIDControllerApp);

ZWPIDControllerApp::~ZWPIDControllerApp()
{
...
void ZWPIDControllerApp::initialize(int stage)
{
...
void ZWPIDControllerApp::initializeCSVFile()
{
    std::time_t now = std::time(nullptr);
    char filename[100];
    std::strftime(filename, sizeof(filename), "pid_controller_log_%Y%m%d_%H%M%S.csv", std::localtime(&now));
...
void ZWPIDControllerApp::handleMessageWhenUp(cMessage *msg)
{
...
void ZWPIDControllerApp::socketDataArrived(UdpSocket *socket, Packet *packet)
{
    processPacket(packet);
}

void ZWPIDControllerApp::socketErrorArrived(UdpSocket *socket, Indication *indication)
{
    EV_WARN << "Socket error arrived" << endl;
    delete indication;
}

void ZWPIDControllerApp::socketClosed(UdpSocket *socket)
{
    EV_WARN << "Socket closed" << endl;
}

void ZWPIDControllerApp::processPacket(Packet *pk)
{
    ...
    EV_INFO << "Received state from destination " << destIndex << ": x=" << currentState[0] << ", theta=" << currentState[2] << endl;

    // Calculate LQG cost (simplified version)
    double lqgCost = currentState[0]*currentState[0] + 100*currentState[2]*currentState[2];  // Example cost function
...
void ZWPIDControllerApp::sendUpdate(int destIndex)
{
    auto packet = new Packet("ZWPIDControllerData");
...
void ZWPIDControllerApp::handleTimeout()
{
...
void ZWPIDControllerApp::logData(simtime_t currentTime, int destIndex, double controllerRTT, double lqgCost)
{
    if (csvFile.is_open()) {
...
void ZWPIDControllerApp::handleStartOperation(LifecycleOperation *operation)
{
    // Controller doesn't initiate communication, it waits for sensor data
}

void ZWPIDControllerApp::handleStopOperation(LifecycleOperation *operation)
{
    cancelEvent(timeoutTimer);
    socket.close();
}

void ZWPIDControllerApp::handleCrashOperation(LifecycleOperation *operation)
{
    cancelEvent(timeoutTimer);
    if (socket.isOpen())
        socket.destroy();
...
void ZWPIDControllerApp::finish()
{
