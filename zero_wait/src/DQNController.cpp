...
class DQNControllerApp : public ApplicationBase, public UdpSocket::ICallback
{
protected:
    UdpSocket socket;
...
Define_Module(DQNControllerApp);

DQNControllerApp::~DQNControllerApp()
{
...
void DQNControllerApp::initialize(int stage)
{
...
void DQNControllerApp::initializeCSVFile()
{
...
void DQNControllerApp::handleMessageWhenUp(cMessage *msg)
{
...
void DQNControllerApp::socketDataArrived(UdpSocket *socket, Packet *packet)
{
    processPacket(packet);
}

void DQNControllerApp::socketErrorArrived(UdpSocket *socket, Indication *indication)
{
    EV_WARN << "Socket error arrived" << endl;
    delete indication;
}

void DQNControllerApp::socketClosed(UdpSocket *socket)
{
    EV_WARN << "Socket closed" << endl;
}

void DQNControllerApp::processPacket(Packet *pk)
{
...
void DQNControllerApp::sendUpdate(int destIndex)
{
    auto packet = new Packet("DQNControllerData");
...
void DQNControllerApp::handleTimeout()
{
...
void DQNControllerApp::logData(simtime_t currentTime, int destIndex, double controllerRTT, double lqgCost)
{
    if (csvFile.is_open()) {
...
void DQNControllerApp::handleStartOperation(LifecycleOperation *operation)
{
    // Controller doesn't initiate communication, it waits for sensor data
}

void DQNControllerApp::handleStopOperation(LifecycleOperation *operation)
{
    cancelEvent(timeoutTimer);
    socket.close();
}

void DQNControllerApp::handleCrashOperation(LifecycleOperation *operation)
{
    cancelEvent(timeoutTimer);
    if (socket.isOpen())
        socket.destroy();
